...
/**
    ...
    :returns: A result type created by wrapping the returned optional.
**/
public func try<T: AnyObject>(function: StaticString = __FUNCTION__, file: StaticString = __FILE__, line: UWord = __LINE__, @noescape wrapError transform: (NSError -> NSError) = identityError, @noescape fn: NSErrorPointer -> T?) -> ObjectResult<T> {
    var err: NSError?
    switch (fn(&err), err) {
    case (.Some(let value), _):
        return success(value)
...
**/
public func try<T: AnyObject>(function: StaticString = __FUNCTION__, file: StaticString = __FILE__, line: UWord = __LINE__, @noescape wrapError transform: (NSError -> NSError) = identityError, @noescape fn: (AutoreleasingUnsafeMutablePointer<T?>, NSErrorPointer) -> Bool) -> ObjectResult<T> {
    var value: T?
    var err: NSError?
